...
public class Homework1 {
	public static void main(String[] args) {
        ...
        for(int i = 0; i < C.length; i++)
        {
          ...
          {
            C[i][j] = A[i][j]^B[i][j];//Виконуємо xor між матрицями А та В
            System.out.print(C[i][j] +" ");
          }
          System.out.println();
        }
        long rez = 0;//змінна для пошуку суми найбільших елементів у кожному рядку
        long max;//Змінна для пошуку найбільшого елемента в рядку
        for(int i = 0; i < C.length; i++)
        {
        	max = C[i][0];
        	for(int j = 1; j < C[i].length; j++)
        	{
        		if (max < C[i][j])
        		{
        			max = C[i][j];//Пошук максимального значення у кожному рядку
        		}
        	}
        	rez += max;//Сума найбільших елементів у кожному рядку 
        }
